...
const useKeywordCostHooks = () => {
    ...
    const updateKeywordCost = (newCost) => {
        loading.setValue(true);
        updateSettingMiddleware("mint keyword cost", SettingType.MINT_PRICE, SettingName.KEYWORD, { value: newCost }, (status, value) => {
    ...
    React.useEffect(() => {
        loading.setValue(true);
        getSettingMiddleware(SettingType.MINT_PRICE, SettingName.KEYWORD, (status, value) => {
            if (status === STATUS_RESPONSE_CODE.SUCCESS && value) {
                keywordCost.setValue(value);
            }
            loading.setValue(false);
