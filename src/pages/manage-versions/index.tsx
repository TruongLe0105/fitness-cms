...
type VersionDialog = "add" | "view" | "edit" | "delete" | "";
...
const ManageVersionPage = () => {
  ...
  const [type, setType] = React.useState<VersionDialog>("");
  const [selected, setSelected] = React.useState<ManageVersion | null>(null);

  const onOpenDialog = (item: ManageVersion, newType: VersionDialog) => {
    setSelected(item);
    setType(newType);
  };
  ...
  return (
    ...
    >
      <Table
        limit={limit.value}
        page={page.value}
        countItems={totalVersion}
        headers={renderHeader(
          (item) => onOpenDialog(item, "view"),
          (item) => onOpenDialog(item, "edit"),
          (item) => onOpenDialog(item, "delete")
        )}
