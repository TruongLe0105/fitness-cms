...
import { useBoolean, useSelectDateRanges, useString, useTable, } from "helpers/hooks";
...
import { emptyReportDetail } from "./types";
import { dataHeaderReport } from "./utils";
import { ReactComponent as IconRemove } from "assets/images/icons/remove-filter.svg";
import { definedDateRanges, FILED_DATE_RANGES, } from "components/DateRanges/types";
import { sumBy } from "lodash";
import ViewDetail from "./organisms/ViewDetail";
import Axios from "axios";
const ReportPage = () => {
    const { isLoadingPage, handleChangePage, handleChangeSort, isLoadingTable, limit, orderBy, orderDirection, page, total, } = useTable();
    const { startDate, endDate, activeSelect, setActiveSelect, setEndDate, setStartDate, openFormDate, } = useSelectDateRanges();
    const [reports, setReports] = useState([]);
    const [reportDetail, setReportDetail] = useState(emptyReportDetail);
    ...
    const handleViewDetailTable = (dataItem) => {
    ...
    return (<PageLayout title="Reports" childrenAction={<div className="flex items-center justify-between h-full pr-8">
          <div></div>
          <div className="flex items-center">
            <DateRangesDefault activeSelect={activeSelect} startDate={startDate} endDate={endDate} setActiveSelect={setActiveSelect} setEndDate={setEndDate} setStartDate={setStartDate} setLoadingTablePage={isLoadingTable.setValue} openFormDate={openFormDate}/>
          </div>
        </div>}>
      <div className="h-40-custom">
        <div className="flex items-center h-8 card-show-filter-item mr-4" style={{
            width: "fit-content",
        }}>
          ...
          {activeSelect !== FILED_DATE_RANGES.WEEK ? (<IconRemove className="ml-4 cursor-pointer" onClick={handleRemoveCustomTime}/>) : null}
        </div>
      </div>
      <div className="custom-height-table-filter">
        <Table limit={limit.value} page={page.value} countItems={total.value} headers={dataHeaderReport(sumListingFee.value, sumDonationFee.value)} handleChangePage={handleChangePage} data={reports.length ? reports : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} isLoadingTable={isLoadingTable.value} hidePagination={true} handleViewDetailTable={handleViewDetailTable}/>
        {isViewDetail.value ? (<ViewDetail dataItem={reportDetail} openPopup={isViewDetail.value} onClose={handleCloseViewDetail}/>) : null}
