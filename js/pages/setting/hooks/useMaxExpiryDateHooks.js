...
const useMaxExpiryDateHooks = () => {
    ...
    const updateMaxExpiryDate = (newValue) => {
        loading.setValue(true);
        updateSettingMiddleware("max expiry date", SettingType.MINT_PRICE, SettingName.MAX_EXPIRY_DATE, { value: newValue }, (status, value) => {
    ...
    React.useEffect(() => {
        loading.setValue(true);
        getSettingMiddleware(SettingType.MINT_PRICE, SettingName.MAX_EXPIRY_DATE, (status, value) => {
            if (status === STATUS_RESPONSE_CODE.SUCCESS && value) {
                maxExpiryDate.setValue(value);
            }
            loading.setValue(false);
