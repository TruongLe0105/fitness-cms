...
const RewardHistoryPage = (): JSX.Element => {
  ...
  const [formDataRewardHistory, setFormDataRewardHistory] =
    useState<RewardHistoryDetail>(emptyRewardHistoryDetail);
  ...
  const getRewardHistory = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: RewardHistoryDetail) =>
    () => {
      console.log("key:", key);
  ...
  return (
    ...
    >
      <Table
        ...
        data={rewardHistory.length ? rewardHistory : []}
