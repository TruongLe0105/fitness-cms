...
const useMinPriceHooks = () => {
    ...
    const updateMinPrice = (newCost) => {
        loading.setValue(true);
        updateSettingMiddleware("min price", SettingType.MINT_PRICE, SettingName.MIN_PRICE, { value: newCost }, (status, value) => {
    ...
    React.useEffect(() => {
        loading.setValue(true);
        getSettingMiddleware(SettingType.MINT_PRICE, SettingName.MIN_PRICE, (status, value) => {
            if (status === STATUS_RESPONSE_CODE.SUCCESS && value) {
                minPrice.setValue(value);
            }
            loading.setValue(false);
