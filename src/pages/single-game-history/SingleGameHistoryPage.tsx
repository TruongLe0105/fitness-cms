...
import {
  ...
  UserDetail,
  RewardHistoryDetail,
  emptyRewardHistoryDetail,
  SingleGameHistoryDetail,
  emptySingleGameHistoryDetail,
} from "./types";
import {
  getNotificationsMiddleware,
  getSingleGameHistoryMiddleware,
} from "./services/api";
...
const SingleGameHistoryPage = (): JSX.Element => {
  const [notifications, setNotifications] = useState<NotificationDetail[]>([]);
  // const [rewardHistory, setRewardHistory] = useState<RewardHistoryDetail[]>([]);
  const [singleGameHistory, setSingleGameHistory] = useState<
    SingleGameHistoryDetail[]
  >([]);
  ...
  const [formDataSingleGameHistory, setFormDataSingleGameHistory] =
    useState<SingleGameHistoryDetail>(emptySingleGameHistoryDetail);
  ...
  const getSingleGameHistory = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
      ...
      const dataRes = await getSingleGameHistoryMiddleware(params, source);
      if (dataRes?.data?.length) {
        setSingleGameHistory(dataRes.data);
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: SingleGameHistoryDetail) =>
    () => {
      console.log("key:", key);
      ...
      setFormDataSingleGameHistory(value);
      switch (key) {
        case "edit":
  ...
  return (
    ...
    >
      <Table
        ...
        data={singleGameHistory.length ? singleGameHistory : []}
...
export default SingleGameHistoryPage;
