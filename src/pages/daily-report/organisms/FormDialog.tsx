...
const FormDialog = (props: FormDialogProps): JSX.Element => {
  ...
  const [selectToSend, setSelectToSend] = useState<SelectToSendDetail>(
  ...
  useEffect(() => {
    ...
    const newType = ListOptionToSendSelect.find(
      (el) => el.value === dataItem.type
    );
  ...
  const onSubmit = () => {
    ...
    if (checkedTimeline.value) {
      ...
      if (timeSentAt < timeNow) {
        showNotification(
          "error",
          "Schedule-Timeline must be greater than the current time!"
        );
        return;
      }
    ...
    if (dataItem.id) {
      updateNotificationMiddleware(
        dataItem.id,
        request,
        (status: STATUS_RESPONSE_CODE, dataRes?: NotificationDetail) => {
    ...
    addNotificationMiddleware(request, (status: STATUS_RESPONSE_CODE) => {
      isLoading.setValue(false);
  ...
  return (
    <DialogCard
      ...
      disablePopup
      title={`${dataItem.id ? "Update" : "Add"} notification`}
    >
      <InputDefault
        label="Name"
        required
      ...
      <div className="flex items-center mb-6">
        <CheckedDefault
          disabled={dataItem.id ? true : false}
          checked={checkedTimeline.value}
          onClick={handleChangeCheckedTimeline}
        />
        <Typography
          fontWeight="font-semibold"
          textColor="text-gray-custom"
          textClass="text-xs ml-3"
        >
          Schedule-Timeline
        </Typography>
      </div>
      {checkedTimeline.value ? (
        <InputDatePicker
          label=""
          rootClass="mb-14"
          inputDate={sentAt}
          setInputDate={setSentAt}
        />
      ) : null}
