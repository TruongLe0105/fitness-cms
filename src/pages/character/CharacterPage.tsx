...
const CharacterPage = (): JSX.Element => {
  ...
  const [formDataCharacter, setFormDataCharacter] =
    useState<CharacterDetail>(emptyCharacterDetail);
  ...
  const getCharacter = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: CharacterDetail) =>
    () => {
      console.log("key:", key);
  ...
  return (
    ...
    >
      <Table
        ...
        data={character.length ? character : []}
