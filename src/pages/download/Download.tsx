...
import {  getDownloadMiddleware, getNotificationsMiddleware } from "./services/api";
...
const DownloadPage = (): JSX.Element => {
  ...
  const [download, setDownload] = useState<DownloadDetail[]>([]);
  ...
  const [formDataDownload, setFormDataDownload] =
    useState<DownloadDetail>(emptyDownloadDetail);
  ...
  const getDownload = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
      ...
      const dataRes = await getDownloadMiddleware(params, source);
      console.log("download - dataRes:", dataRes);
      setDownload(dataRes.items);
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: DownloadDetail) =>
    () => {
      console.log("key:", key);
      if (key !== "view-detail" && value.id) {
        return;
      }
      setFormDataDownload(value);
      switch (key) {
        case "edit":
