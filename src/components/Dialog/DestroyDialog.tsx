...
const DestroyDialog = (props: DeleteDialogProps): JSX.Element => {
  const { url, label, message, handleUpdateWhenDestroy, onClose, openPopup } =
    props;
  const isLoading = useBoolean();

  const onSubmitDestroy = () => {
    isLoading.setValue(true);
    deleteMiddleware(url, message, (status: STATUS_RESPONSE_CODE) => {
