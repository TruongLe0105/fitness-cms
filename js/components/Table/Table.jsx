...
const Table = (props) => {
    ...
    const renderTableBody = (header, value, index) => {
        ...
        if (header.isCheckbox && !value.nftId) {
            return (<CheckedDefault checked={value.checked ?? false} onClick={handleChangeChecked &&
    ...
    return (<div className="relative" style={{
        ...
        <table className="table-card">
          ...
          <tbody>
            ...
            return (<tr key={index} onClick={onClickViewDetail(el)} className={`${handleViewDetailTable ? "cursor-pointer" : ""}`} style={{
                    ...
                    {headers.map((header, idx) => (<td key={idx} style={header.styleBody}>
      ...
      <div className=" flex items-center justify-end bg-white pr-8" style={{
        ...
        {data.length && (<PaginatedItems countItems={countItems} limit={limit} page={page} handleChangePage={handleChangePage}/>)}
      </div>
