...
const VersionPage = (): JSX.Element => {
  ...
  const [formDataVersion, setFormDataVersion] =
    useState<VersionDetail>(emptyVersionDetail);
  ...
  const getVersion = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
      ...
      const dataRes = await getVersionMiddleware(params, source);
      console.log("dataRes:", dataRes);
      setVersion(dataRes.items);
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: VersionDetail) => () => {
      console.log("key:", key);
      if (key !== "view-detail" && value.id) {
        return;
      }
      setFormDataVersion(value);
      switch (key) {
        case "edit":
