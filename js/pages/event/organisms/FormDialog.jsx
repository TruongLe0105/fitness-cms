...
const FormDialog = (props) => {
    ...
    const onSubmit = () => {
        ...
        if (!dataItem.id && timeStartDate < timeNow) {
            showNotification("error", "Start Date must be greater than the current time!");
            isError = true;
        }
        if (timeStartDate > timeEndDate) {
            showNotification("error", "End date should be greater or equal to Start date!");
            isError = true;
        }
        ...
        if (dataItem.id) {
            updateEventMiddleware(dataItem.id, formData, (status, dataRes) => {
        ...
        addEventMiddleware(formData, (status) => {
            isLoading.setValue(false);
    ...
    const fileSelectedImageURL = (event) => {
    ...
    return (<DialogCard openPopup={openPopup} handleCLoseDialog={onClose} disablePopup title={`${dataItem.id ? "Update" : "Add"} event`} rootStyle={{
            width: 800,
        }}>
      <div className="grid" style={{
            gridTemplateColumns: "359px calc(100% - 359px)",
        }}>
        <ImageEventCard fileInput={fileInput} fileSelectedImageURL={fileSelectedImageURL} keyInputFile={keyInputFile.value} handleRemoveFileInput={handleRemoveFileInput} originImage={dataItem.url}/>
        <div className="flex flex-col">
          <InputDefault label="Name" required rootClass="mb-6" value={name.value} onChange={handleChangeInputName}/>
          <MultipleInput label="Description" rootClass="mb-6" rows={5} value={description.value} onChange={handleChangeInput("description")}/>
          {/* <MultipleInput
          label="Target URL"
          rootClass="mb-6"
          rows={3}
        ...
        /> */}
          ...
          <InputDatePicker label="Start date*" rootClass="mb-6" inputDate={startDate} setInputDate={setStartDate}/>
          <InputDatePicker label="End date*" rootClass="mb-14" inputDate={endDate} setInputDate={setEndDate}/>
        </div>
      </div>

      <ButtonDefault widthButton="w-140-custom mt-8" disabled={isDisabledButton()} onClick={onSubmit}>
