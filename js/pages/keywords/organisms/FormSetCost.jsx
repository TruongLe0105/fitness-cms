...
const FormSetCost = (props) => {
    const { dataItem, openForm, onClose, updateListKeyword } = props;
    const [fromDataSetCost, setFromDataSetCost] = useState(null);
    const [originFromDataSetCost, setOriginFromDataSetCost] = useState(null);
    useEffect(() => {
    ...
    const onSubmitButton = async () => {
        ...
        const body = Object.keys(formDataReduce).map((key) => ({
            id: key,
    ...
    const handleChangeInput = (id, valueInput) => {
        setFromDataSetCost({
            ...fromDataSetCost,
            [id]: valueInput,
        });
    ...
    const disabledButton = () => {
        ...
        let isDisable = false;
        if (Object.values(fromDataSetCost).filter((item) => Number(item) > 0)
    ...
    const compareFormDataSetCost = () => {
        return reduce(fromDataSetCost, function (result, value, key) {
    ...
    return (<Popup disabled className="dialog" open={openForm}>
      <div className="view-dialog-body">
        <div className="flex items-center justify-between mb-5">
          ...
          <img src={IconClose} alt="icon" className="cursor-pointer" onClick={onClose}/>
        </div>
        <div className="flex flex-col p-4 card-info-star">
          <div className="flex h-50-custom border-b border-gray-02-custom">
            {headers.map((header, ind) => (<div key={ind} className="w-300-custom px-4 flex items-center ">
                <Typography variant="h6" textColor="text-black" fontWeight="font-semibold">
                  {header}
                </Typography>
              </div>))}
          </div>
          {dataItem.keywords.length
            ? dataItem.keywords
                ...
                .map((el, index) => (<div key={index} className={`flex h-50-custom ${index < dataItem.keywords.length - 1
                    ...
                    <div className="w-300-custom px-4 flex items-center ">
                      <CustomCurrencyInput value={fromDataSetCost ? fromDataSetCost[el.id] : ""} onValueChange={(value) => handleChangeInput(el.id, value)}/>
                    </div>
                  </div>))
            : null}
        </div>
        <div className="flex items-center justify-between mt-8">
          <ButtonDefault widthButton="w-140-custom" onClick={onSubmitButton} disabled={disabledButton()}>
            Submit
          </ButtonDefault>
        </div>
