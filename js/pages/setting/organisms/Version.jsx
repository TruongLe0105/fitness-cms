...
const Version = (props) => {
    ...
    const item = React.useRef(defaultInputs);
    ...
    const isDisabledButton = () => {
        const { name: versionName, link, versionCode, buildNumber, isRequired, } = inputs;
        if (!versionName ||
            !link ||
    ...
    const onSubmitButton = () => {
        ...
        isLoading.setValue(true);
        updateSettingStringMiddleware("version", SettingType.VERSION, name, { value: JSON.stringify(value) }, (status, res) => {
    ...
    return (<div style={{
      ...
      <InputDefault label="Name" required value={inputs.name} onChange={handleChangeInput("name")} inputStyle={{ marginBottom: 20 }}/>
      <div style={{ marginBottom: 20 }}>
        <InputDefault label="Link" required value={inputs.link} onChange={handleChangeInput("link")}/>
        {error ? (<p style={{ fontSize: 12, color: "rgba(239, 68, 68)" }}>{error}</p>) : ("")}
      </div>
      <InputDefault label={isAndroid ? "Version code" : "Build number"} required value={(isAndroid ? inputs.versionCode : inputs.buildNumber) || ""} onChange={handleChangeInput(isAndroid ? "versionCode" : "buildNumber")} inputStyle={{ marginBottom: 20 }} type="number"/>
      <div style={{
            display: "flex",
            alignItems: "center",
            marginBottom: 40,
        }}>
        <CheckedDefault title="Required update" checked={inputs.isRequired} onClick={() => setInputs({
            ...inputs,
            isRequired: !inputs.isRequired,
        })}/>
        <p style={{
