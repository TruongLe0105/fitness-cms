...
import { emptyNotificationDetail, emptyUserDetail, ListOptionToSendSelect, ListSentSelect, TYPE_NOTIFICATION, emptyTransactionDetail, } from "./types";
import { getNotificationsMiddleware, getTransactionMiddleware, } from "./services/api";
...
const TransactionPage = () => {
    ...
    const [formDataTransaction, setFormDataTransaction] = useState(emptyTransactionDetail);
    ...
    const getTransaction = async (source) => {
        try {
            const params = {
                limit: limit.value,
                page: page.value,
    ...
    return (<PageLayout title="Transaction" childrenAction={<div className="flex items-center justify-between h-full pr-8">
          {/* <ButtonDefault widthButton="w-140-custom" onClick={handleOpenFormAdd}>
              Add notification
            </ButtonDefault> */}

          <div className="flex items-center">
            <FilterTable placeholder="from" search={search.value} handleChangeInputSearch={handleChangeInputSearch}/>
          </div>
        </div>}>
    ...
    data={transaction.length ? transaction : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} isLoadingTable={isLoadingTable.value}/>
