...
import { emptyNotificationDetail, emptyUserDetail, ListOptionToSendSelect, ListSentSelect, TYPE_NOTIFICATION, } from "./types";
import { getNotificationsMiddleware, getUsersMiddleware } from "./services/api";
...
const UserPage = () => {
    ...
    const getUsers = async (source) => {
        try {
            ...
            console.log('hello');
            const dataRes = await getUsersMiddleware(params, source);
            setUsers(dataRes.items);
            console.log('dataRes.total:', dataRes.total);
            total.setValue(dataRes.total);
            cleanStateRequest();
        }
        catch (error) {
    ...
    data={users.length ? users : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} isLoadingTable={isLoadingTable.value}/>
