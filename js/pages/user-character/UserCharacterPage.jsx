...
import { emptyNotificationDetail, emptyUserCharacterDetail, emptyUserDetail, ListOptionToSendSelect, ListSentSelect, TYPE_NOTIFICATION, } from "./types";
import { getNotificationsMiddleware, getUserCharactersMiddleware, } from "./services/api";
...
const UserCharacterPage = () => {
    ...
    const getUserCharacters = async (source) => {
        try {
            const params = {
                limit: limit.value,
                page: page.value,
    ...
    return (<PageLayout title="User-Character" childrenAction={<div className="flex items-center justify-between h-full pr-8">
    ...
    data={userCharacters.length ? userCharacters : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} isLoadingTable={isLoadingTable.value}/>
