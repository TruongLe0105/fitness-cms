...
const AdvertisementPage = () => {
    ...
    const { handleChangePage, limit, orderBy, orderDirection, page, total, isLoadingPage, isLoadingTable, handleChangeSort, } = useTable('createdAt', ORDER_DIRECTION.ASC);
    ...
    const getSystemAdvertisement = async (source) => {
        try {
            const params = {
                limit: limit.value,
                page: page.value,
    ...
    return (<PageLayout title='Advertisement' childrenAction={<div className='flex items-center justify-between h-full pr-8'>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <ButtonDefault widthButton='w-180-custom' onClick={() => {
      ...
      <div>
        <TableCreateAdvertisement limit={limit.value} page={page.value} countItems={total.value} headers={dataHeaderAdvertisement()} handleChangePage={handleChangePage} data={systemAdvertisement.length ? systemAdvertisement : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} checkedAdd={checkedAll.value} isLoadingTable={isLoadingTable.value}/>
      </div>

      {openFormCreateAdvertisement.value && (<FormCreateAdvertisement openForm={openFormCreateAdvertisement.value} onClose={onCloseDialog} item={selected} headers={dataHeaderAdvertisement()} countItems={total.value} orderDirection={orderDirection.value} onRefetch={onRefetch}/>)}
    </PageLayout>);
};
