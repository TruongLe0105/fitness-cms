...
const MenuItemCard = (props) => {
    ...
    return (<div onClick={handleMenuPage} className={`h-50-custom pl-8 flex items-center relative mb-3 cursor-pointer hover-menu-item ${activeMenu ? "bg-primary-custom content-menu-item svg-icon-menu" : ""}`}>
      {renderIconMenu()}
      {openMenu ? (<Typography variant="h6" textColor={activeMenu ? "text-primary-custom" : "text-gray-custom"} fontWeight="font-medium" textClass="ml-7 custom-white-space">
          {menu.label}
        </Typography>) : null}
    </div>);
};
