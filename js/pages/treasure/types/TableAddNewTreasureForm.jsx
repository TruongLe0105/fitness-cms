...
const TableAddNewTreasureForm = (props) => {
    const { limit, page, countItems, handleChangePage, headers, data, orderBy, orderDirection, handleChangeSort, checkedAdd, handleChangeChecked, isLoadingTable, hidePagination, handleViewDetailTable, rowStyle, openForm, openChecked, } = props;
    ...
    const renderIconSort = (newField) => {
        ...
        return (<div className='p-1 ml-2 rounded-full bg-gray-02-custom cursor-pointer icon-sort' onClick={handleChangeSort && handleChangeSort(newField, ORDER_DIRECTION.ASC)}>
        <img src={IconArrowUp} alt='icon' className='cursor-pointer w-15-custom h-15-custom'/>
      </div>);
    };
    const onClickViewDetail = (dataItem) => (event) => {
    ...
    return (<div className='relative' style={{
      ...
      <div style={{
        ...
        {openForm && (<div className='text-xl text-gray-custom'>
        ...
        <table className='table-card over'>
          {!openForm ? (<thead>
              <tr>
                {headers.map((header, idx) => idx !== 0 && ( //disable thumbnail header column,cause reuse
            <th key={idx} style={{
                    ...header.styleHeader,
                }}>
          ...
          <tbody>
            ...
            // console.log(el.name);
            return (<tr key={index} onClick={onClickViewDetail(el)} className={`${handleViewDetailTable ? 'cursor-pointer' : ''}`} style={{
                    borderBottom: index === data.length - 1
                        ? 'none'
                        : '1px solid rgba(0, 0, 0, 0.05)',
                    ...rowStyle,
                }}>
                    {headers.map((header, idx) => openForm
                    ? (idx === 0 || idx === 1) && ( //get 2 first column
                    <td onClick={() => {
                            openChecked(true);
                        }} key={idx} style={header.styleBody}>
