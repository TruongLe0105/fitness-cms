...
import {
  ...
  UserDetail,
  VsGameHistoryDetail,
  emptyVsGameHistoryDetail,
} from "./types";
import {
  getNotificationsMiddleware,
  getVsGameHistoryMiddleware,
} from "./services/api";
...
const VsGameHistoryPage = (): JSX.Element => {
  const [notifications, setNotifications] = useState<NotificationDetail[]>([]);

  const [vsGameHistory, setVsGameHistory] = useState<VsGameHistoryDetail[]>([]);
  ...
  const [formDataVsGameHistory, setFormDataVsGameHistory] =
    useState<VsGameHistoryDetail>(emptyVsGameHistoryDetail);
  ...
  const getVsGameHistory = async (source?: CancelTokenSource) => {
    try {
      const params: ParamsRequest = {
        limit: limit.value,
        page: page.value,
      ...
      const dataRes = await getVsGameHistoryMiddleware(params, source);

      if (dataRes?.data?.length) {
        setVsGameHistory(dataRes.data);
  ...
  const handleOpenUpdateList =
    (key: "edit" | "delete" | "view-detail", value: VsGameHistoryDetail) =>
    () => {
      console.log("key:", key);
      ...
      setFormDataVsGameHistory(value);
      switch (key) {
        case "edit":
  ...
  return (
    ...
    >
      <Table
        ...
        data={vsGameHistory.length ? vsGameHistory : []}
...
export default VsGameHistoryPage;
