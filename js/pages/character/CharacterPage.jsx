...
import { emptyNotificationDetail, emptyUserDetail, ListOptionToSendSelect, ListSentSelect, TYPE_NOTIFICATION, emptyCharacterDetail, } from "./types";
import { getCharacterMiddleware, getNotificationsMiddleware } from "./services/api";
...
const CharacterPage = () => {
    ...
    const [formDataCharacter, setFormDataCharacter] = useState(emptyCharacterDetail);
    ...
    const getCharacter = async (source) => {
        try {
            const params = {
                limit: limit.value,
                page: page.value,
            ...
            const dataRes = await getCharacterMiddleware(params, source);
            console.log("dataRes:", dataRes);
            setCharacter(dataRes.items);
            total.setValue(dataRes.total);
            cleanStateRequest();
        }
        catch (error) {
    ...
    return (<PageLayout title="Character" childrenAction={<div className="flex items-center justify-between h-full pr-8">
          ...
          <div className="flex items-center">
            ...
            /> */}
            <FilterTable placeholder="name" search={search.value} handleChangeInputSearch={handleChangeInputSearch}/>
          </div>
        </div>}>
    ...
    data={character.length ? character : []} handleChangeSort={handleChangeSort} orderBy={orderBy.value} orderDirection={orderDirection.value} isLoadingTable={isLoadingTable.value}/>
