...
import { postNewStar, updateImageStarMiddleware, getStarDetailMiddleware, } from '../services/api';
import '../styles/index.css';
import { useState, useEffect } from 'react';
import { STATUS_RESPONSE_CODE } from 'types';
import { cloneDeep } from 'lodash';
import { defaultAddTreasureInput } from '../types';
...
export const FormAddNewTreasure = ({ openForm, onClose, item, headers, dataItem, onRefresh, handleChangeInputSearch, }) => {
    const [treasureDetail, setTreasureDetail] = useState(dataItem);
    ...
    const fileSelectedHandler = (event) => {
        ...
        updateImageStarMiddleware(dataItem.id, {
    ...
    const onSubmitButton = () => {
        isLoading.setValue(true);
        const { name, description, starId, type, tokenContract, total, imgUrl } = treasureDetail;
        const request = {
            name,
            description,
            tokenContract,
    ...
    const onSelectChangeType = (value) => {
        setTreasureDetail({ ...treasureDetail, type: value.value });
    };
    const onSelectChangeTokenContract = (value) => {
        setTreasureDetail({
            ...treasureDetail,
            tokenContract: value.value,
        });
    };
    const isDisabled = !treasureDetail?.imgUrl ||
        !treasureDetail?.name ||
        !treasureDetail?.description ||
        !treasureDetail?.tokenContract ||
        !treasureDetail?.total ||
        !treasureDetail?.starId;
    return (<DialogCard openPopup={openForm} disablePopup handleCLoseDialog={onClose} title={item ? 'Update treasure' : `Add new treasure`} size='md' rootStyle={{
            width: '90vw',
            maxHeight: '90vh',
            overflow: 'auto',
        }}>
      ...
      <div className='grid grid-cols-5 mb-6 gap-5'>
        <InputDefault label='Name' required value={treasureDetail.name} onChange={handleChangeInput('name')} placeholder='Sun' inputStyle={inputStyle}/>
        <InputDefault label='Description' value={treasureDetail.description} onChange={handleChangeInput('description')} placeholder='' inputStyle={inputStyle}/>
        <SelectDefault label='Token Contract' required options={tokenContractOpitons} handleChange={onSelectChangeTokenContract} styleControl={inputStyle} styleSingleValue={{
            display: 'flex',
            alignItems: 'center',
            fontSize: 14,
            maxWidth: 'inherit',
        }} controlWidth={1}/>
        <InputDefault label='Total Reward' value={treasureDetail.total} onChange={handleChangeInput('total')} placeholder='' inputStyle={inputStyle}/>
        <SelectDefault label='Treasure Type' required options={TYPE} handleChange={onSelectChangeType} styleControl={inputStyle} styleSingleValue={{
            display: 'flex',
            alignItems: 'center',
            fontSize: 14,
            maxWidth: 'inherit',
      ...
      <div className='mb-6 mt-10 flex justify-between w-full' style={{ display: 'flex' }}>
        <div className='flex  items-center border-2' style={{ padding: 10 }}>
          <div className='treasure-show'>
            {starDetail.id ? (<div style={{
                ...
                height: '40px',
                borderRadius: '50%',
            }} src='https://i.pinimg.com/originals/66/31/a8/6631a844feea41e8b0100d6512a11e04.jpg' alt='default planet'/>)}
          ...
          <button onClick={() => openFormSelectTreasure.setValue(true)} className='border-2 rounded cursor-pointer py-3 px-10'>
        ...
        <div className='relative'>
          ...
          <React.Fragment>
            <input id='container-files' onChange={fileSelectedHandler} accept={'image*'} type='file' style={{ display: 'none' }} key={keyInputFile.value || ''}/>
            <label htmlFor='container-files'>
              <div>
                <img src={IconAddAvatar} alt='icon' className='absolute bottom-2 right-2 cursor-pointer'/>
              </div>
            </label>
          </React.Fragment>
        </div>
        <FormSelectNewTreasure openForm={openFormSelectTreasure.value} onClose={onCloseDialog} headers={headers} handleChange={(idTemp) => {
        ...
        }} starId={treasureDetail.starId} handleChangeInputSearch={handleChangeInputSearch}/>
      </div>
      <ButtonDefault widthButton='w-140-custom' onClick={onSubmitButton} disabled={isDisabled} style={{
            minHeight: 37,
        }}>
        {item ? 'Update' : 'Add'}
      </ButtonDefault>
